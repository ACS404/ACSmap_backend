...
class CancerRiskAPI:
    ...
    class _Predict(Resource):
        """Predict overall cancer risk and optional per-cancer-type breakdown."""
        ...
        def post(self):
            try:
                ...
                # Optional list of cancer types to assess (from frontend chip selection)
                selected_cancer_types = patient_data.pop('selected_cancer_types', None)
                ...
                # Only run per-type prediction when the user has chosen types
                if selected_cancer_types is not None:
                    response['cancer_type_risks'] = model.predict_cancer_types(
                        patient_data, selected_types=selected_cancer_types
                    )
        ...
        def get(self):
            try:
                ...
                return {
                    ...
                    'cancer_types':     list(CancerRiskModel.CANCER_TYPES_META.keys()),
                    'supported_values': {
                        'sex':                ['male', 'female'],
    ...
    class _CancerTypes(Resource):
        """List all supported cancer types and their metadata."""

        def get(self):
            return {'cancer_types': CancerRiskModel.CANCER_TYPES_META}, 200
...
api.add_resource(CancerRiskAPI._CancerTypes,        '/cancer-types')