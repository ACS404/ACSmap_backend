...
**Your backend is running on:** `localhost:8009` ✅
...
**Change the port from `8585` to `8009`:**
...
// AFTER (Correct):
const javaURI = "http://localhost:8009";
```
...
export const javaURI = "http://localhost:8009";  // ← Make sure it's 8009!
...
Edit `assets/js/api/config.js` and change `8585` → `8009`
...
```bash
curl http://localhost:8009/api/post/all
# Should return: []
```
...
| Service | Port | Status |
|---------|------|--------|
| **Flask Backend** | **8009** | ✅ Correct - Use this! |
...
1. **Backend is on 8009** - This is confirmed and working ✅
2. **Don't change backend** - It's correct!
3. **Change frontend config** - Update to port 8009
...
Once you update the port to `8009`:
...
```javascript
// Backend API Configuration
export const javaURI = "http://localhost:8009";
...
# Find and replace 8585 with 8009
find assets -name "*.js" -type f -exec sed -i '' 's/8585/8009/g' {} +
...
| What | Where | Change |
|------|-------|--------|
| **Backend** | Flask (running) | Port **8009** ✅ |
| **Frontend Config** | `config.js` | Change to **8009** ❌→✅ |
...
Just update that one line in your frontend config from `8585` to `8009` and everything will work!
